...
// Big cute cartoon SVG illustrations — pets & owners traveling across SA
function TravelScene({ dark }) {
  ...
  return (
    <svg viewBox="0 0 1400 200" preserveAspectRatio="xMidYMid slice" fill="none" xmlns="http://www.w3.org/2000/svg"
      ...
      {/* Big warm sun with face */}
      <g transform="translate(1250, 32)" className="scene-sun">
        <circle r="34" fill="url(#sun)" opacity="0.9" />
        {/* Rays */}
        ...
        {/* Cute face on sun */}
        <circle cx="-8" cy="-4" r="3" fill="#E8A020" /> {/* eye */}
        <circle cx="8" cy="-4" r="3" fill="#E8A020" />
        <circle cx="-8" cy="-5" r="1.5" fill="#2C2418" /> {/* pupil */}
        <circle cx="8" cy="-5" r="1.5" fill="#2C2418" />
        <circle cx="-7" cy="-6" r="0.6" fill="white" /> {/* shine */}
        <circle cx="9" cy="-6" r="0.6" fill="white" />
        <path d="M-8 5 Q0 12 8 5" stroke="#E8A020" strokeWidth="2" fill="none" strokeLinecap="round" /> {/* smile */}
        <circle cx="-14" cy="2" r="4" fill="#FFE0A0" opacity="0.3" /> {/* blush */}
        <circle cx="14" cy="2" r="4" fill="#FFE0A0" opacity="0.3" />
      </g>

      {/* Clouds */}
      {[{x:180,y:28,s:1},{x:500,y:18,s:0.7},{x:850,y:35,s:0.9},{x:1100,y:20,s:0.6}].map((c,i) => (
        <g key={`cloud-${i}`} className="scene-clouds" transform={`translate(${c.x},${c.y}) scale(${c.s})`} opacity={dark ? 0.15 : 0.5}>
          <ellipse cx="0" cy="0" rx="30" ry="12" fill="white" />
          <ellipse cx="-18" cy="4" rx="18" ry="10" fill="white" />
          <ellipse cx="18" cy="4" rx="18" ry="10" fill="white" />
          <ellipse cx="0" cy="6" rx="28" ry="10" fill="white" />
        </g>
      ))}

      {/* Table Mountain silhouette (iconic flat top) */}
      ...
      {/* Acacia trees (bigger, more detailed) */}
      ...
      {/* Protea flowers (national flower of SA) */}
      ...
      {/* === SCENE 1: Cute bakkie with boerboel & jack russell in back === */}
      <g transform="translate(60, 100)" className="scene-bakkie">
        {/* Bakkie body (chunky, cute proportions) */}
        ...
        <circle cx="35" cy="25" r="1.5" fill="#FFD166" /> {/* tag */}

        {/* Jack Russell sitting next to boerboel (small & cute) */}
        ...
        <circle cx="51" cy="21" r="1.2" fill="#2C2418" /> {/* nose */}
        <path d="M49 22 Q51 23.5 53 22" stroke="#2C2418" strokeWidth="0.6" fill="none" /> {/* smile */}
        {/* Perky ears */}
        <path d="M47 15 L48 18 L45 17Z" fill="#C08040" />
        <path d="M54 15 L53 18 L56 17Z" fill="#C08040" />

        {/* Wheels (chunky) */}
        <circle cx="18" cy="62" r="9" fill={dark ? '#2A2018' : '#3A3028'} />
        <circle cx="18" cy="62" r="4" fill={dark ? '#5A4A38' : '#6A5A48'} />
        <circle cx="18" cy="62" r="1.5" fill={dark ? '#7A6A58' : '#8A7A68'} />
        <circle cx="72" cy="62" r="9" fill={dark ? '#2A2018' : '#3A3028'} />
        <circle cx="72" cy="62" r="4" fill={dark ? '#5A4A38' : '#6A5A48'} />
        <circle cx="72" cy="62" r="1.5" fill={dark ? '#7A6A58' : '#8A7A68'} />
      ...
      {/* === SCENE 2: Kid on bicycle with sausage dog in sidecar === */}
      <g transform="translate(320, 106)" className="scene-bicycle">
        {/* Sidecar (little cart) */}
        ...
        {/* Sausage dog (dachshund) in sidecar — LONG & cute */}
        ...
        <ellipse cx="11" cy="31" rx="2" ry="1.5" fill="#2C2418" /> {/* nose */}
        <path d="M7 34 Q9 36 11 34" stroke="#2C2418" strokeWidth="0.6" fill="none" /> {/* smile */}
        {/* Floppy long ears */}
        ...
        {/* Front paws on sidecar rim */}
        <ellipse cx="2" cy="38" rx="2.5" ry="1.5" fill="#8B5A30" />
        <ellipse cx="8" cy="38" rx="2.5" ry="1.5" fill="#8B5A30" />
        {/* Goggles! */}
        <ellipse cx="3" cy="28" rx="3" ry="2.5" fill="none" stroke={dark ? '#8B6B42' : '#C4A060'} strokeWidth="1.2" />
        <ellipse cx="8" cy="28" rx="3" ry="2.5" fill="none" stroke={dark ? '#8B6B42' : '#C4A060'} strokeWidth="1.2" />
        <line x1="6" y1="28" x2="5" y2="28" stroke={dark ? '#8B6B42' : '#C4A060'} strokeWidth="1" />

        {/* Bicycle */}
        <circle cx="20" cy="56" r="10" fill="none" stroke={dark ? '#6A5A48' : '#5A4A38'} strokeWidth="2.5" />
        <circle cx="20" cy="56" r="2.5" fill={dark ? '#6A5A48' : '#5A4A38'} />
        <circle cx="50" cy="56" r="10" fill="none" stroke={dark ? '#6A5A48' : '#5A4A38'} strokeWidth="2.5" />
        <circle cx="50" cy="56" r="2.5" fill={dark ? '#6A5A48' : '#5A4A38'} />
        {/* Frame */}
      ...
      <g transform="translate(560, 118)" className="scene-meerkats">
        {/* Meerkat 1 — standing tall, lookout pose */}
        <ellipse cx="0" cy="30" rx="5" ry="8" fill="#D4B080" />
        <circle cx="0" cy="18" r="6" fill="#D4B080" />
        <ellipse cx="0" cy="32" rx="4" ry="2" fill="#C4A070" /> {/* belly */}
        {/* Face */}
        <ellipse cx="0" cy="19" rx="3.5" ry="3" fill="#E0C8A0" /> {/* face mask */}
        <circle cx="-2" cy="17" r="1.8" fill="#2C2418" /> {/* eye */}
        <circle cx="2" cy="17" r="1.8" fill="#2C2418" />
        <circle cx="-2" cy="16.5" r="0.6" fill="white" />
        <circle cx="2" cy="16.5" r="0.6" fill="white" />
        {/* Dark eye patches (classic meerkat!) */}
        <ellipse cx="-2" cy="17" rx="2.5" ry="2.2" fill="#8A6A40" opacity="0.4" />
        <ellipse cx="2" cy="17" rx="2.5" ry="2.2" fill="#8A6A40" opacity="0.4" />
        <ellipse cx="0" cy="20" rx="1.5" ry="1" fill="#2C2418" /> {/* nose */}
        <path d="M-1.5 22 Q0 23.5 1.5 22" stroke="#2C2418" strokeWidth="0.5" fill="none" />
        {/* Tiny ears */}
        <circle cx="-4" cy="14" r="1.5" fill="#C4A060" />
        <circle cx="4" cy="14" r="1.5" fill="#C4A060" />
        {/* Little arms */}
        <path d="M-4 26 L-6 22" stroke="#C4A070" strokeWidth="2" strokeLinecap="round" />
        <path d="M4 26 L6 22" stroke="#C4A070" strokeWidth="2" strokeLinecap="round" />
        {/* Feet */}
        <ellipse cx="-3" cy="38" rx="3" ry="1.5" fill="#B09060" />
        <ellipse cx="3" cy="38" rx="3" ry="1.5" fill="#B09060" />

        {/* Meerkat 2 — smaller baby, peeking */}
      ...
      {/* === SCENE 4: Family hiking — mom, kid, and big ridgeback + cat in bubble backpack === */}
      <g transform="translate(700, 82)" className="scene-hikers">
        {/* Mom */}
        ...
        <ellipse cx="23" cy="30.5" rx="0.8" ry="1.5" fill="#2C2418" /> {/* slit pupil */}
        ...
        <path d="M25 33 L24.5 33.5 L25.5 33.5Z" fill="#FFB0B0" /> {/* pink nose */}
        {/* Cat whiskers */}
        ...
        <path d="M21.5 25 L22.5 27 L20 26Z" fill="#FFB0C0" /> {/* inner ear */}
        ...
        <circle cx="52" cy="23" r="2" fill={dark ? '#4A7C59' : '#5A9A6E'} /> {/* pompom */}
        {/* Face */}
        ...
        {/* Ridgeback running ahead — bigger, bouncier */}
        ...
        <ellipse cx="106" cy="42" rx="2.5" ry="1.8" fill="#2C2418" /> {/* nose */}
        {/* Happy face */}
      ...
      {/* === SCENE 5: VW Kombi (SO South African!) with surfboard & parrot === */}
      <g transform="translate(1050, 96)" className="scene-kombi">
        {/* Kombi body */}
        ...
        {/* Parrot on roof! (bright SA colors) */}
        ...
        {/* Wheels */}
        <circle cx="16" cy="62" r="9" fill={dark ? '#2A2018' : '#3A3028'} />
        <circle cx="16" cy="62" r="4" fill={dark ? '#5A4A38' : '#6A5A48'} />
        <circle cx="58" cy="62" r="9" fill={dark ? '#2A2018' : '#3A3028'} />
        <circle cx="58" cy="62" r="4" fill={dark ? '#5A4A38' : '#6A5A48'} />

        {/* Peace sign flower sticker on side */}
      ...
      {[{x:280,y:20},{x:290,y:15},{x:300,y:20},{x:295,y:10},{x:310,y:16},{x:960,y:18},{x:970,y:12},{x:980,y:18}].map((b,i) => (
        <path key={`b-${i}`} d={`M${b.x-5} ${b.y} Q${b.x} ${b.y-4} ${b.x+5} ${b.y}`}
          stroke={sky1} strokeWidth="1.2" fill="none" />
      ))}

      {/* Paw prints on the road */}
